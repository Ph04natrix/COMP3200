%% ----------------------------------------------------------------
%%? (1st Draft Complete)
%%* IMPORTANT: Make sure that this references some of the background
%% ----------------------------------------------------------------
\chapter{Introduction}% \label{Chapter:Introduction}
\section{Problem}
As technology has advanced over the years, people's personal music collections have become more and more digital, with "around 2 in 3 people listening to music using a streaming service" \cite{streaming::percentage}.
...
This hierarchical system allows for the better handling of large scale collections but if these playlists grow too large, then they require reorganising which is a mentally taxing activity. As such the organisational benefit provided by these playlists is lost due to the 'perceived' high mental load required. Whilst the capability and format of personal music collections have changed dramatically with technological advancements, the structure of these collections has stagnated in the streaming era.
...
This project will employ two different organisational models:\begin{itemize}
    %\item \textbf{Clustered Table}
    \item \textbf{Static Cartesian Graphs} \(\to\) each song is mapped onto a cartesian grid (both 2D and 3D) where the co-ordinates of the song is determined by the numerical attribute for that axis
    \item \textbf{Dynamic Similarity Graph} \(\to\) each song is a node in a graph with edges between each song for each attribute and metadata. As such songs will be 'pulled' closer to together if they have a high overall similarity.
\end{itemize}
